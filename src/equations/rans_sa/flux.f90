...
REAL,DIMENSION(CONS,0:Nloc,0:Nloc,0:ZDIM(Nloc)),INTENT(IN)  :: U        !< Conservative solution
REAL,DIMENSION(PRIM,0:Nloc,0:Nloc,0:ZDIM(Nloc)),INTENT(IN)  :: UPrim    !< Primitive solution
!> Physical fluxes in x,y,z directions
REAL,DIMENSION(CONS,0:Nloc,0:Nloc,0:ZDIM(Nloc)),INTENT(OUT) :: f,g,h
!----------------------------------------------------------------------------------------------------------------------------------
! LOCAL VARIABLES
INTEGER             :: i,j,k
!==================================================================================================================================
...
f(MUSA) = -1./sigma*(muS+muTilde*SAfn)*gradUx(LIFT_NUSA)   ! F_euler-1/sigma*(muS+muTilde)*nuTilde_x
...
g(MUSA) = -1./sigma*(muS+muTilde*SAfn)*gradUy(LIFT_NUSA)   ! F_euler-1/sigma*(muS+muTilde)*nuTilde_y
...
h(MUSA) = -1./sigma*(muS+muTilde*SAfn)*gradUz(LIFT_NUSA)   ! F_euler-1/sigma*(muS+muTilde)*nuTilde_z
#else
! gradients of primitive variables are directly available gradU = (/ drho, dv1, dv2, dv3, dT /)
