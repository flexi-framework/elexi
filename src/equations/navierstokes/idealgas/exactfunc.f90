...
CASE(33) !Roundjet, x-axis is jet axis
  ...
  prim(VELV)  = 0.
  prim(PRES)  = RefStatePrim(5,RefState)
  prim(TEMP)  = prim(PRES)/(prim(DENS)*R)
  ! Jet inflow (from x=0)
...
CASE(1340) ! blasius in z direction
  ...
  x_eff=x+x_offset
  ! blasius BL for pipes
  x_eff(2)=JetRadius-SQRT(x(1)**2+x(2)**2)+x_offset(2)
  IF(x_eff(2).GT.0 .AND. x_eff(3).GT.0 .AND. x(3).LT.0.098) THEN
    ! scale bl position in physical space to reference space, eta=5 is ~99% bl thickness
    eta=x_eff(2)*(prim(DENS)*prim(VEL3)/(mu0*x_eff(3)))**0.5
