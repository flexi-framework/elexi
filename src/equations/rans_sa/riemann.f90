...
ELSE
  ...
  IF ((Ssl .LE. 0.).AND.(SStar .GE. 0.)) THEN
    ...
    F(1:5)=F_L(1:5)+Ssl*(U_Star(1:5)-U_LL(1:5))
  ELSE
    ...
    F(1:5)=F_R(1:5)+Ssl*(U_Star(1:5)-U_RR(1:5))
  END IF
...
REAL                    :: SqrtRho_L,SqrtRho_R,sSqrtRho
REAL                    :: RoeVel(3),RoeH,Roec,absVel
REAL,DIMENSION(1:5)     :: a,r1,r2,r3,r4,r5  ! Roe eigenvectors
...
REAL,DIMENSION(1:5)     :: r1,r2,r3,r4,r5,a,al,ar,Delta_U,Alpha  ! Roe eigenvectors
REAL                    :: tmp,da
!=================================================================================================================================
c_L       = SPEEDOFSOUND_HE(U_LL)
c_R       = SPEEDOFSOUND_HE(U_RR)
...
! subsonic case
ELSE
  F(1:5)=(Ssr*F_L(1:5)-Ssl*F_R(1:5)+Ssl*Ssr*(U_RR(1:5)-U_LL(1:5)))/(Ssr-Ssl)
END IF ! subsonic case
...
REAL,DIMENSION(1:5)                    :: r2,r3,r4  ! Roe eigenvectors + jump in prims
!=================================================================================================================================
H_L       = TOTALENTHALPY_HE(U_LL)
H_R       = TOTALENTHALPY_HE(U_RR)
SqrtRho_L = SQRT(U_LL(DENS))
...
ELSE
  ...
  F(1:5)=(Ssr*F_L(1:5)-Ssl*F_R(1:5) + Ssl*Ssr* &
     (U_RR(1:5)-U_LL(1:5) - delta*(r2*Alpha(2)+r3*Alpha(3)+r4*Alpha(4))))/(Ssr-Ssl)
END IF ! subsonic case
